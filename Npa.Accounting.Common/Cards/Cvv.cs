using System;
using System.Text.RegularExpressions;

namespace Npa.Accounting.Common.Cards
{
    public record Cvv
    {
        private static readonly Regex regex = new Regex(@"^\d{3}$", RegexOptions.Compiled | RegexOptions.Singleline);

        public Cvv(string Value)
        {
            var value = string.IsNullOrWhiteSpace(Value) ? throw new ArgumentNullException(nameof(Value)) : Value.Trim();
            if (!IsValid(value))
            {
                throw new FormatException($"Cvv '{Value}' is in an incorrect format.");
            }

            this.Value = value;
        }

        public string Value { get; }
        public static implicit operator string(Cvv n) => n.ToString();
        
        public override string ToString() => Value;

        private static bool IsValid(string value) => regex.IsMatch(value);
    }
}